...
ZONE = "KE"
N_X, N_Y = 1, 1               # 1 coc truoc (kiem chung co che) — sau moi scale len 9 (3x3)
...
msh_path = ROOT / "scratch" / "test_ops_gd5.msh"
gmsh.initialize()
...
VFO_MODEL, VFO_CASE = "ke_gd5_staged", "GD_full"
...
for stage_i, stage in enumerate(params.stages, start=1):
    ...
    print(f"[{stage.name}] q={stage.q_avg_kPa:.2f}kPa footprint={stage.load_footprint} "
          f"ecc={stage.eccentricity_m}m n_nut_tai={n_loaded} -> analyze return={ok}")

    # Chuyen vi ngang tai dinh cot (node dau tien trong TOP_COLUMN)
    top_col_nodes = np.unique(nsets.get("TOP_COLUMN", np.array([])))
    if len(top_col_nodes) > 0:
        n0 = int(top_col_nodes[0]) + 1
        ux = ops.nodeDisp(n0, 1)
        uz = ops.nodeDisp(n0, 3)
        print(f"  Ux dinh cot = {ux*1000:.2f} mm, Uz = {uz*1000:.2f} mm")
...
vfo.plot_deformedshape(model=VFO_MODEL, loadcase=VFO_CASE, scale=20, contour="x",
                        setview="3D", filename=str(IMG_DIR / "vfo_ke_gd5_deformed"))
