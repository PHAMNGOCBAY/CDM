...
@media print {
  /* Override containers — bắt buộc để content chảy nhiều trang.
     KHÔNG dùng height:auto trong wildcard * vì:
       - MathJax CHTML dùng inline height cho subscript/superscript/fraction
       - Plotly dùng absolute height cho chart canvas
     height:auto chỉ áp dụng cho Streamlit scroll containers bên dưới. */
  * {
  ...
  /* ── KaTeX / MathJax: KHÔNG override height/position ──
     Glyph stack của math cần height cố định để align dấu Việt + subscript.
     Wildcard * { height: auto } phía trên làm dấu/subscript bay loạn → restore. */
  .katex, .katex *,
  .katex-display, .katex-display *,
  .katex-html, .katex-html *,
  span.katex, span.katex *,
  mjx-container, mjx-container *,
  mjx-math, mjx-math *,
  .MathJax, .MathJax *,
  ...
  /* ẨN .katex-mathml — đây là MathML annotation cho screen reader,
     KHÔNG phải để hiển thị. Khi position: revert ở rule trên làm nó
     visible → mỗi công thức bị duplicate (HTML version + MathML version). */
  .katex-mathml,
  .katex .katex-mathml,
  span.katex-mathml,
        ...
        for _hk_iter in _hk_d_list:
            with st.expander(f"Chi tiết KE-{_hk_iter}", expanded=False):
                ...
                try:
                    ...
                    with st.spinner("Đang chạy kiểm tra Lật + Xoay nhổ chân cừ..."):
