...
def _resolve_db_path(db_path=None) -> str:
    """Trả về path tới TTHC.sqlite.

    Thứ tự ưu tiên:
    1. Tham số db_path truyền vào
    2. Env var TTHC_DB_PATH (cho phép port sandbox 8504 dùng DB riêng)
    3. Mặc định: data/TTHC.sqlite cạnh repo
    """
    if db_path:
        return str(db_path)
    import os
    env_path = os.environ.get("TTHC_DB_PATH", "").strip()
    if env_path:
        return env_path
