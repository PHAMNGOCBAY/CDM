...
### 44. Module cdm3d — Mô hình 3D Trụ CDM–Đất nền (Gmsh + CalculiX, LOCAL ONLY)

**File tài liệu:** [76-cdm3d-fem-gmsh-calculix.md](76-cdm3d-fem-gmsh-calculix.md)
**Package:** `scripts/cdm3d/` (types, params_io, geometry, mesh_gmsh, ccx_input, run_ccx, postprocess)
**Demo:** `python scripts/cdm3d_demo.py KE` (hoặc `BXN`/`NHC`)
**Config:** `data/cdm3d_params.json` (config — D/spacing/pattern lấy từ `tvtk_cdm_202605_TTHC.json`, `assumed_defaults` là giá trị giả định PHẢI kỹ sư xác nhận)
**Output:** `results/cdm3d/<zone>.msh|.inp|.frd` (không commit) · `images/cdm3d_<zone>_*.png`

**Pipeline:** Gmsh OCC kernel dựng khối đất 3 lớp (đất đắp/đất yếu/lớp cứng) + nhóm trụ CDM hình trụ → boolean fragment → mesh tứ diện (Box field mịn quanh trụ) → xuất `.msh` → ghi deck CalculiX `.inp` → giải bằng `ccx.exe` (subprocess) → `ccx2paraview` đọc `.frd` → `pyvista` render PNG 3D.

**Đã test thành công toàn bộ pipeline (2026-08-26)** kể cả giải thật: hình học + lưới (28818 nút/172358 phần tử) → CalculiX (`C:\CalculiX\calculix_2.23_4win\ccx_static.exe` — đã có sẵn trên máy, không cần DLL ngoài) → ảnh kết quả 3D. `ccx.exe` là binary biên dịch sẵn (Fortran/C) — KHÔNG có bản thuần Python nào giải được FEM này (các gói Python `gmsh`/`ccx2paraview` chỉ sinh lưới/đọc kết quả, không tự giải).

**Lỗi đã gặp và fix khác:** (1) `ccx2paraview` v3.2.0 crash khi đổi `.frd`→`.vtu` do dùng `np.linalg.eigvals` (không phải `eigvalsh`) cho tensor đối xứng, sai số làm tròn sinh trị riêng phức → vá runtime trong `postprocess._patch_ccx2paraview_principal_stress()`. (2) Bật `*DLOAD GRAV` (trọng lượng bản thân) khi CHƯA có bước khởi tạo geostatic K0 cho lún SAI HOÀN TOÀN (test ra ~0,9m — vô lý, do áp trọng lực từ trạng thái ứng suất=0). Đã đổi mặc định `include_self_weight=False` trong `ccx_input.write_ccx_inp()` — chuyển vị tính được chỉ là lún do tải đắp `q`, so sánh được trực tiếp với S1 TCVN 9403. Kết quả test KE: lún đỉnh max ≈ 19,8cm (giữa nhóm trụ) vs S1 1D = 10,3cm — chênh lệch hợp lý, phản ánh lún cục bộ đất giữa trụ lớn hơn mức trung bình composite 1D.

**Lỗi đã gặp và fix — phân loại mặt biên KHÔNG được dùng tâm bounding-box:** mặt `TOP_SOIL` là 1 mặt phẳng nhiều lỗ tròn (multiply-connected) nơi trụ xuyên qua — tâm bbox của nó trùng đúng toạ độ 1 trụ khi lưới trụ đặt tại gốc toạ độ (0,0), gây phân loại nhầm thành `TOP_COLUMN`. Phải dùng **diện tích** (`gmsh.model.occ.getMass(2, tag)`, mặt đỉnh trụ ≈ π(D/2)²) để phân biệt. Áp dụng nguyên tắc này cho mọi bài toán gmsh OCC fragment có mặt nhiều lỗ trong tương lai.

**GIF:** `postprocess.render_mesh_gif()` — xoay camera 360° (chưa có trường kết quả để hoạt hình màu). `render_results_gif()` — theo yêu cầu người dùng, **camera CỐ ĐỊNH**, thay vào đó hoạt hình hoá tải tăng dần 0→100% (30 khung): nhân đồng thời vector chuyển vị (biến dạng hình học) và giá trị màu với hệ số tải mỗi khung (hợp lệ vì đàn hồi tuyến tính), `clim` cố định để phổ màu "sáng dần" thay vì nhảy. `cdm3d_demo.py` xuất cả PNG+GIF mặc định (`--no-gif` để tắt).

**Đơn giản hoá V1 (ghi rõ trong doc, không được bỏ qua khi đọc kết quả):** vật liệu đàn hồi tuyến tính (chưa Mohr-Coulomb), liên kết trụ-đất bonded (chưa contact/trượt), tải đắp quy đổi lực nút xấp xỉ (chưa `*DLOAD` áp lực mặt chuẩn), chưa có bước khởi tạo ứng suất geostatic K0 — chuyển vị kết quả là **tương đối** (do tải thêm), không phải trạng thái tuyệt đối tại chỗ.

**LOCAL ONLY** — giống `fem2d` (mục 13b): phụ thuộc `gmsh`/`pyvista` (nặng, cần OpenGL) + binary ngoài `ccx.exe`. KHÔNG thêm vào whitelist `update_app.bat`, KHÔNG deploy Cloud.

**Giai đoạn thi công + tải lệch tâm (2026-08-27):** mặc định mọi lần giải chạy chuỗi 6 bước GD(-1) dummy → GD0 (REMOVE cọc) → GD1 (ADD STRAIN FREE) → GD2..N (đắp + lệch tâm tăng dần) — xem `76-cdm3d-fem-gmsh-calculix.md` mục 8. **2 lỗi CalculiX quan trọng đã phát hiện qua thực nghiệm (không có trong tài liệu chính thức):** (1) file `.frd` chỉ ghi mesh MỘT LẦN — nếu `*MODEL CHANGE REMOVE` là thao tác đầu tiên, mesh "đóng băng" thiếu phần tử đó VĨNH VIỄN kể cả sau khi ADD lại → PHẢI có 1 bước "mọi phần tử active" đứng trước mọi REMOVE đầu tiên; (2) `ADD=STRAIN FREE` bắt buộc `*STEP, NLGEOM`. Công thức tải lệch tâm tổng quát dùng hệ số **12** (không phải 6 — hệ số 6 chỉ đúng khi đánh giá tại mép x=B/2), và PHẢI tính diện tích tributary THẬT từ tam giác bề mặt (không chia đều theo số nút — lưới không đều quanh cọc sẽ pha loãng độ lệch tâm). Trích xuất chuyển vị ngang qua `column_forces.extract_lateral_profile()` (pyvista `sample_over_line`) — đã kiểm chứng cho kết quả trơn tru hợp lý. Mô men uốn tương đương (`extract_moment_profile()`) ĐÃ VIẾT nhưng kết quả còn nhiễu (lưới quanh cọc quá thô) — CHƯA đủ tin cậy để dùng.
