...
**Quy tắc cao độ QTT (toggle, mặc định bật):** hố có **CĐTN > 1 m → lấy CĐTN = 1 m** (san/đào về mốc +1 m) → tải đắp q = γ·(CĐTK − 1); **CĐTK theo hình** = bản đồ phân vùng cao độ thiết kế. CĐTK mặc định mỗi hố lấy từ `data/qtt_cdtk_design.json` (ND-02=3,30 · ND-03=3,05 · ND-04=2,80 · ND-05=2,80 · ND-06=3,05 · ND-07=3,30) — **ưu tiên hơn** điểm lưới `qtt_elevation_points` gần nhất; sửa được trong bảng data_editor. Nhờ vậy ND-05/06/07 (CĐTN cao +3,2..+4,24) có tải đắp thay vì H=0.

**Địa tầng QTT — nguồn chuẩn mới (cập nhật 2026-06-05):** Toàn bộ địa tầng 6 hố ND-02..07 đọc lại từ file hình trụ mới `HÌNH TRỤ, MC-QTT/QTT-7 HO KHOAN 20260605.dxf` (6 cột hố cạnh nhau, cách 230 đơn vị). Script [scripts/qtt_relayer_import.py](scripts/qtt_relayer_import.py) — trích marker (số hiệu lớp @cột trái + BỀ DÀY) + mô tả gần nhất → cộng dồn top/bot; XÓA lớp cũ, ghi cả 2 DB (LOCAL+PROJECT) + JSON `data/qtt_nd0X_borehole.json`. **5 đơn vị địa chất chuẩn:** F=Đất san lấp · 1=(CH) Sét rất dẻo "chảy xen kẹp dẻo chảy-dẻo mềm" · 2=(CH) Sét rất dẻo "dẻo mềm" · 3=(SM) Cát lẫn bụi chặt vừa · 4=**(CL,CH) Sét ít dẻo lẫn rất dẻo "dẻo cứng đến nửa cứng"** (đất CỨNG, e₀<1 → Eoed, KHÔNG cố kết). Lớp đất yếu cũ "Bùn sét chảy" nay là "(CH) Sét rất dẻo" (vẫn e₀≥1, cố kết). Tổng bề dày mỗi hố khớp chiều sâu (ND-02=31; ND-03/05/06/07=36; ND-04=33, KHÔNG có lớp F). DWG không đọc trực tiếp được — phải xuất DXF (AutoCAD `DXFOUT`). `soil_type_of` phân loại theo TRẠNG THÁI: "dẻo cứng/nửa cứng/cứng" → "Sét (dẻo cứng)" (cứng), KHÔNG gọi nhầm "Sét dẻo cao (CH)" mềm dù mô tả có "(CH)"/"rất dẻo".

**Quy tắc tham số QTT theo LOẠI ĐẤT (BẮT BUỘC — sửa 2026-06):** ký hiệu lớp QTT đánh số **tuần tự per-hố KHÔNG đồng nhất** (số 2 hố này = bùn, hố khác = cát) → nếu lấy chỉ tiêu trung bình theo ký-hiệu sẽ **trộn loại đất** → lớp cát/sét cứng bị gán e₀/Cc của bùn → cả profile thành "sét cố kết dày thoát 1 mặt" ảo (ND-04 U15 10,6%, ND-06 9,1% — SAI). Fix: `settle_avg` với zone=="QTT" dùng `representative_params_by_soil("QTT")` — chỉ tiêu gom theo **loại đất** (`soil_type_of` từ mô tả); `is_sand` cũng theo loại đất (`{Đá san lấp, Cát, Cát pha}`). KE/BXN/NHC giữ nguyên rep theo ký hiệu (ký hiệu địa chất thống nhất). Sau fix: ND-04 36%, ND-06 18% (đúng vật lý).

**Biên thoát nước theo LỚP THỰC (không theo phân tố):** `settle_avg` trả `perm_layers` = lớp thấm RAW từ địa tầng gốc; `time_history` dùng `_perm_below/_perm_above` để bắt cả **lớp cát/cát pha MỎNG < bề dày phân tố 2 m** (vd ND-05 cát pha 1,8 m ở 33–34,8 m bị chia phân tố bỏ sót → trước nhận nhầm 1 mặt). Kết quả: đáy cụm sét có cát → 2 mặt; đáy là **sét pha dẻo cứng (aquitard, không cát)** → 1 mặt (ND-03/06/07 chậm thật, đúng vật lý).
